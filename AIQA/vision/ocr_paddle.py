...
class OCR:
    def __init__(self, lang="en", use_gpu=False):
        ...
        self._ocr = PaddleOCR(lang=lang, use_gpu=use_gpu, show_log=False, use_angle_cls=True)
    ...
    def words(self, img: Image.Image, roi: Optional[Box] = None) -> List[dict]:
        ...
        arr=np.asarray(crop); res=self._ocr.ocr(arr, cls=True)
        if not res: return []
        lines = res[0] if isinstance(res,list) else res
        out=[]
        for item in lines or []:
            poly=item[0]; txt=(item[1][0] or "").strip()
            if not txt: continue
